...
class ListDetector:
    ...
    def identify(self, blocks: list[Block]) -> None:
        ...
        修复（2026-08-18，RPS系统需求规范书 RQ-010/012/013 折半问题）：
        原实现只要 list 区域有 ≥1 项就识别为列表，导致正文行内"B）"等
        被误识为列表 marker —— 整段连续正文被切成 1 个 list item + 1 个
        intro，下游渲染时被当作列表按行输出，句子折半。
        修复：要求 list 区域 ≥2 个有效 item（_build_tree 返回 ≥2 项），
        并要求 _split_regions 的 marker 必须是常见列表标记（避免单字母+）
        在正文括号内的偶发匹配，如 "(A、\nB）"）。
        """
        for b in blocks:
            ...
            if not list_regions:
                continue
            # 验证：至少有一个 list region 包含 ≥2 个有效 item
            has_real_list = any(len(r.get("items", [])) >= 2 for r in list_regions)
            if not has_real_list:
                continue
            # 验证：所有 list region 的 marker 必须是真正常见的列表标记
            # 排除单字母+右括号（容易与正文括号匹配混淆，如 "(A、\nB）"）
            COMMON_MARKERS = ("numeric", "circled", "cn_num", "bullet", "dash", "dash_line")
            all_markers_common = all(
                it.get("type") in COMMON_MARKERS
                for r in list_regions
                for it in r.get("items", [])
            )
            if not all_markers_common:
                continue
            b.type = "list"
